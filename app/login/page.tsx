...
export default function LoginPage() {
  ...
  async function submitAuth() {
    ...
    localStorage.setItem("couple-space:remember-me", rememberMe ? "true" : "false");
  ...
  return (
    <main className="relative flex min-h-screen items-center justify-center overflow-hidden bg-[#fff1f4] px-4 py-24 text-[#3b0710] transition-colors dark:bg-[#090205] dark:text-white sm:px-6 lg:px-10">
      <div
      ...
      <section className="relative z-10 grid w-full max-w-6xl items-center gap-8 lg:grid-cols-[1.04fr_0.96fr]">
            ...
            <div className="relative grid gap-4">
              <div className="rounded-[1.5rem] bg-white/60 p-5 shadow-inner dark:bg-white/10">
                <div className="flex items-center justify-between">
                  <div>
                    ...
                    <p className="mt-1 text-4xl font-black">547 дней</p>
                  </div>
              ...
              <div className="grid grid-cols-2 gap-4">
                {[
                  ...
                  ["📸", "Воспоминания", "124 момента"],
                  ["🏆", "Достижения", "18 открыто"],
        ...
        <div className="auth-reveal auth-reveal-delay mx-auto w-full max-w-md">
          <div className="mb-6 text-center lg:hidden">
            <div className="mx-auto flex h-20 w-20 items-center justify-center rounded-[1.6rem] bg-white/55 text-5xl shadow-2xl ring-1 ring-white/60 backdrop-blur dark:bg-white/10 dark:ring-white/10">
              ❤️
            </div>
          ...
          <div className="overflow-hidden rounded-[2rem] border border-white/50 bg-white/45 p-4 shadow-[0_32px_110px_rgba(127,29,29,0.24)] backdrop-blur-2xl dark:border-white/10 dark:bg-white/8">
            <div className="rounded-[1.7rem] bg-white/55 p-5 shadow-inner dark:bg-black/20 sm:p-6">
              <div className="mb-5 grid grid-cols-2 rounded-full bg-[#fff1f4]/80 p-1 shadow-inner dark:bg-white/10">
              ...
              <div key={`${mode}-${method}`} className="auth-form-switch space-y-3">
                {method === "email" ? (
                  <AuthInput
                    value={email}
                    onChange={setEmail}
                  ...
                  <div className="group relative">
                    <input
                      ...
                      placeholder="Логин"
                      className="h-16 w-full rounded-[1.35rem] border border-white/45 bg-white/65 px-5 text-base font-bold text-[#3b0710] outline-none shadow-[inset_0_1px_0_rgba(255,255,255,0.65),0_18px_45px_rgba(127,29,29,0.08)] backdrop-blur transition placeholder:text-[#9f1239]/45 focus:border-[#fb7185]/70 focus:bg-white/85 focus:shadow-[0_0_0_5px_rgba(251,113,133,0.16),0_22px_55px_rgba(190,18,60,0.16)] dark:border-white/10 dark:bg-white/10 dark:text-white dark:placeholder:text-white/35 dark:focus:bg-white/15"
                    />
                  </div>
                )}
              ...
              <label className="mt-5 flex cursor-pointer items-center gap-3 text-sm font-bold text-[#be123c]/75 dark:text-white/65">
                <input
                  type="checkbox"
                  checked={rememberMe}
              ...
              {(errorMessage || successMessage) && (
                <div
                  className={`mt-5 rounded-[1.25rem] p-4 text-sm font-bold shadow-inner ${
                    successMessage
              ...
              <button
                onClick={submitAuth}
                disabled={isLoading}
                className="auth-gradient-button mt-5 flex h-16 w-full items-center justify-center rounded-[1.35rem] px-6 text-base font-black text-white shadow-[0_18px_50px_rgba(220,38,38,0.34)] transition hover:-translate-y-1 hover:scale-[1.015] disabled:cursor-not-allowed disabled:opacity-70"
              >
                {isLoading ? (
