            ...
            <div className="mt-8 rounded-[1.7rem] bg-white/72 p-4 shadow-inner dark:bg-black/18">
              <textarea
                ...
                onChange={(event) => {
                  ...
                  if (event.target.value.trim() !== lastSavedAnswer.trim()) {
                    setSaveStatus("Сохраняем...");
                  }
                }}
                disabled={isEditLocked}
                placeholder="Напиши свой ответ..."
                className="min-h-[220px] w-full resize-none rounded-[1.3rem] border border-emerald-200/80 bg-white/75 p-5 text-lg font-semibold leading-8 text-emerald-950 outline-none placeholder:text-emerald-700/38 focus:border-emerald-400 disabled:cursor-not-allowed disabled:opacity-65 dark:border-white/10 dark:bg-white/8 dark:text-white dark:placeholder:text-white/36"
              />
